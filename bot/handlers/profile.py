...
from client import api, bot, db, wm
from bot.rules import HasPhoto
from bot.keyboard.generators import no_kb, KeyboardSet
...
async def edit_name(chat_id: int):
    await api.send_message(
        chat_id,
        text="Напиши мне твое новое имя",
        reply_markup=KeyboardSet.KEYBOARD_UNDO.get_markup(),
    )
    m, _ = await wm.wait(bot.dispatch.message, (api, chat_id))
    if not m.text or m.text.lower() == "отменить":
        return
    await db.query(
        "update User filter .telegram_id = <str>$telegram_id set {name := <str>$name}",
        telegram_id=str(chat_id),
        name=safety.make_safe(m.text),
    )
    await api.send_message(chat_id, text="Имя изменено.", reply_markup=no_kb)


async def edit_age(chat_id: int):
    await api.send_message(
        chat_id,
        text="Напиши твой возраст, например: 19",
        reply_markup=KeyboardSet.KEYBOARD_UNDO.get_markup(),
    )
    m, _ = await wm.wait(
        ...
        FuncRule(
            ...
            and (
                (msg.text.isdigit() and MIN_AGE <= int(msg.text) <= 100)
                or msg.text.lower() == "отменить"
            ),
            EventAdapter("message", Message),
        ),
        default=MessageReplyHandler(
            f"Пожалуйста напиши возвраст числом, учти, что минимальный возраст {MIN_AGE} лет"
        ),
    )
    ...
    await api.send_message(chat_id, text="Возраст изменен.", reply_markup=no_kb)


async def edit_pictures(chat_id: int):
    await api.send_message(
        chat_id,
        text="Пришли новую фотографию для своего профиля.",
        reply_markup=KeyboardSet.KEYBOARD_UNDO.get_markup(),
    )
    ...
    while len(photos) < 3:
        ...
        if len(photos) < 3:
            await api.send_message(
                chat_id,
                text="Добавил, пришли мне еще, если хочешь добавить "
    ...
    await api.send_message(chat_id, text="Фотографии успешно изменены.")


async def edit_description(chat_id: int):
    await api.send_message(
        chat_id,
        text="Пришли мне новое описание для твоего профиля",
    ...
    await db.query(
        ...
        telegram_id=str(chat_id),
        description=safety.make_safe(description),
    )
    await api.send_message(chat_id, text="Описание изменено.", reply_markup=no_kb)
...
async def edit_gender(chat_id: int):
    ...
    await api.send_message(chat_id, text="Настройки пола изменены.", reply_markup=no_kb)
...
async def edit_interest(chat_id: int):
    ...
    to_edit = (
        await api.send_message(
            chat_id,
            text="Выбери категорию, или категории людей, которые тебе интересны",
            reply_markup=gen_interest_kb(interest),
        )
    ).unwrap()

    # TODO: use telegrinder.Checkbox

    while True:
        q: CallbackQuery
        q, _ = await wm.wait(bot.dispatch.callback_query, (api, to_edit.message_id))
    ...
    await api.send_message(chat_id=chat_id, text="Предпочтения отредактированы.")
...
@dp.callback_query(FuncRule(lambda cb, _: cb.callback_query.data == "profile/edit"))
...
@dp.callback_query(
    FuncRule(lambda cb, _: (cb.callback_query.data or "").startswith("profile/edit/"))
)
async def profile_edit_field(cb: CallbackQuery):
    ...
    await profile(Message(**cb.message.to_dict(), api=cb.ctx_api), u)


@dp.callback_query(
    FuncRule(lambda cb, _: cb.callback_query.data == "profile/toggle_hide")
)
